...
Streaming **ZipCrypto** unzip **and zip** for **Node.js**. Parse a password zip from a `node:stream` `Readable` in one pass (local file headers only), or write one to a `Writable` without buffering file payloads. Entries arrive via `for await`; file bodies are streams. There is no `files[]` list and no central-directory seek on read.

Public API is Node `Readable` / `Writable` only. This package does not export Web Streams and is not a browser bundle.
...
```ts
import { Zip, unzipEncrypted, zipEncrypted } from "@js-ak/zip-password-stream"; // ESM
```
...
const {
  ...
  isPdfMagic,
  isPdfPath,
} = require("@js-ak/zip-password-stream"); // CJS
```

## Usage

The usual read path is `Zip.open(source).password(pwd).only("pdf")`. `unzipEncrypted` is the same parser without the fluent builder. To **create** a zip: `Zip.create(dest).password(pwd)` then `add` / `end` (`zipEncrypted` is the low-level writer).
...
Prefer `.only` / `.filter` (or `filter` on `unzipEncrypted`) to skip unused names when the local header has a size: that discards **compressed** bytes and does not inflate (safer on untrusted zips than `autodrain()`, which decompresses). Encrypted skips still check the ZipCrypto header, so a wrong password fails. Entries written by this library use bit 3 with **no size in the local header** (deflate only): skip then **must inflate** to find the data descriptor — set `maxEntrySize` on untrusted input.
...
`.only("pdf")` is that same basename check (`isKindPath`). It is not magic: a `.pdf` name with non-PDF bytes still yields. Sniff the body with `isPdfMagic` in the pipeline if you need that.
...
`autodrain()` still decrypts and inflates a yielded entry into a sink. Use it only after the entry was yielded; for PDF-only, `.only("pdf")` is the cheap path.
...
### Create a password zip, then open it

`Zip.create` streams deflate (ZipCrypto when `.password` is set). Local crc/sizes are zero; a 16-byte data descriptor follows each body. Call `end()` to write the central directory and wait until `dest` has finished — without `end()`, other tools cannot open the file.

ZipCrypto is traditional PKWARE encryption. This library writes it for **interop** with tools that require a password zip. It is **not** confidentiality-grade (known-plaintext attacks against a few hundred bytes of known content). If you need real protection, encrypt the payload before `add`.
...
`Zip.open` wraps a Node `Readable`. Builder methods do not read the zip; `for await` starts `unzipEncrypted` with the options collected so far.

`Zip.create` wraps a Node `Writable`. Builder methods do not write; the first `add` or `end` starts `zipEncrypted`. The package exports `ZipCreate` as a type only (`export type { ZipCreate }`); the class is not constructible from the package.
...
Low-level writer. Same layout as `Zip.create`.
...
export type ZipWriter = {
  add(name: string, body: Readable): Promise<void>;
  end(): Promise<void>; // CD + EOCD, dest.end(), then wait until dest has finished
};
...
`password` may be omitted (`undefined`) for a fully unencrypted archive; an empty string is a real (empty) ZipCrypto password. An encrypted entry without a password throws `password required: <name>`. Encrypted members (`flags & 1`) use a **new ZipCrypto key state per entry**.
...
`.only("pdf")` calls `isKindPath`; `isPdfMagic` is for the body after yield. These helpers sniff zip **entry** bytes and names so you can filter without trusting `entry.path` alone. They are not a `file-type` replacement: no MIME map, no deep sniff, no extra formats beyond the table.
...
No `unzipper`, `jszip`, or `@zip.js/zip.js`. Compression uses `node:zlib` (`createInflateRaw`, `createDeflateRaw`, `crc32`); everything else comes from `node:stream`, `node:stream/promises`, `node:events`, and `node:crypto` (`randomBytes` for ZipCrypto headers).
